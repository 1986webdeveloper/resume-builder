...
export class ResumeController {
  ...
  static async createUserBasicDetails(
    req: Request,
    res: Response
  ): Promise<Response> {
    ...
    const validation = await userResumeService.checkStepAndRequiredFields(
      userInput.data,
      step
    );
    const sectionData = validation.schemaData;
    let data: any = validation.customPayload;
    const responseData = await userResumeService.createORAddStepData(
      step,
      data,
      userInput.resumeId,
      userInput.userId,
  ...
  static async getResumeInfo(req: Request, res: Response): Promise<Response> {
    ...
    const resumeInfo: any =
      await userResumeService.checkUsercurrentStep(userInput);
    return res.json({ message: Success, data: resumeInfo });
  }
  //#endregion

  //#region  get all resume list
  static async getAllResumeList(
    req: Request,
    res: Response
  ): Promise<Response> {
  ...
  static async createDataTypes(req: Request, res: Response): Promise<Response> {
    ...
    // check validation
    await checkValidation(userInput);
    const typesObj = userResumeService.createDataTypeAllowTypes(
      userInput.allowedValueTypes
    );
    const filter: any = { title: userInput.title };
    const checkExits = await userResumeService.getDataType(filter, ["_id"]);
    if (checkExits)
      throw new HttpError(errorDataTypeAlreadyExist, HTTP_STATUS_CODE.found);
  ...
  static async createResumeSchema(
    req: Request,
    res: Response
  ): Promise<Response> {
    ...
    const filed = {
      ...
      dataTypes: userResumeService.createDataTypeAllowTypes(
        userInput?.dataTypes ?? []
      ),
    };
    let newData;
    ...
    if (userInput.sectionId) {
      ...
      const checkFiledExits =
        await userResumeService.getResumeSchemaAggregate(query);
      if (checkFiledExits)
        throw new HttpError(errorFieldAlreadyExists, HTTP_STATUS_CODE.found);
  ...
  static async editOrDeleteResumeSchema(
    req: Request,
    res: Response
  ): Promise<Response> {
    ...
    //check validation
    await checkValidation(userInput);
    const is_active_delete =
      userInput.active == ACTIVE || userInput.active == DE_ACTIVE;
    const update: any = {};
    ...
    if (userInput.sectionTitle) {
      ...
      if (userInput.fieldName)
        ...
        update["$set"] = {
          "fields.$.dataTypes": userResumeService.createDataTypeAllowTypes(
            userInput?.dataTypes ?? []
          ),
        };
      } else if (is_active_delete)
        update["$set"] = { "fields.$.is_active": userInput.active };
    } else if (is_active_delete) update.is_active = userInput.active;
    const updateRecord = await userResumeService.updateResumeSchema(
      filter,
      update
    );
    if (updateRecord && is_active_delete && !userInput?.fieldId) {
      ...
      if (userInput.active == ACTIVE) updateOrder["$inc"] = { order: 1 };
  ...
  static async changeSectionOrder(
    req: Request,
    res: Response
  ): Promise<Response> {
    ...
    await checkValidation(userInput);
    const filter: any = { _id: userInput.schemaId };
    const checkCurrentStep: any = await userResumeService.getResumeSchema(
      filter,
      ["_id", "order"]
    );
    const currentStepOrder = checkCurrentStep?.order;
    const updateOrder = { order: userInput.order };
    const checkChangeOrder: any = await userResumeService.getResumeSchema(
      updateOrder,
      ["_id", "order"]
    );
    if (checkChangeOrder)
      await userResumeService.updateResumeSchema(
        { _id: checkChangeOrder._id },
  ...
  static async editOrDeleteUserResume(
    req: Request,
    res: Response
  ): Promise<Response> {
    ...
    if (!checkData)
      throw new HttpError(errorResumeNotFound, HTTP_STATUS_CODE.not_found);
    ...
    if (userInput.sectionId) {
      ...
      if (userInput.elementId) {
        ...
        if (is_update_data) {
          //check update fields
          await await userResumeService.checkStepAndRequiredFields(
            userInput.data,
            checkData.steps.step,
            false
          );
          for (let key in userInput.data) {
            updateData[`steps.$[section].data.$[element].${key}`] =
              userInput.data[key];
          }
        } else if (is_active_delete)
          updateData[`steps.$[section].data.$[element].is_active`] =
            userInput.active;
      } else if (is_active_delete)
        updateData["$set"] = { "steps.$[section].is_active": userInput.active };
      else if (is_update_data && userInput.data.skills)
        updateData["$set"] = { "steps.$[section].data": userInput.data.skills };
    } else if (is_active_delete) updateData["is_active"] = userInput.active;
    const update = await userResumeService.updateUserResume(
      update_filter,
      updateData,
      arrayFilters
    );
  ...
  static async stepInfo(req: Request, res: Response): Promise<Response> {
    ...
    const steps = await userResumeService.getResumeSchemaData(
      filter,
      attributes
    );
