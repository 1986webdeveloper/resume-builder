...
export class EducationController {
  ...
  static async addEducationDetails(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<Response> {
    ...
    if (checkExist) {
      ...
      if (performance) {
        let is_found = checkExistingPerformance.find(
          (each) =>
            each.label == performance?.label && each.value == performance?.value
        );

        if (!is_found)
          updateRecord["$push"] = {
            performances: { ...(performance ?? {}), userId },
          };
      }
  ...
  static async editOrDeleteEducation(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<Response> {
    ...
    //update data
    const is_update = await educationService.updateEducation(
      update_where,
      updateData
    );
  ...
  static async getEducationDetails(
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<Response> {
